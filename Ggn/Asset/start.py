...
async def start(event):
    ...
    collection.update_one({"user_id": user_id}, {"$set": {"user_id": user_id}}, upsert=True)
    buttons = [
        [Button.url("Join Channel", url="https://t.me/+bxa7pKmXMCFjMjJl")],
        [Button.url("Contact Me", url="https://t.me/study_ocean_bot")],
    ]
...
S = "/start"
START_PIC = "https://files.catbox.moe/lwd4k1.jpg"
TEXT = """ </blockquote> Hi, welcome to Advance Content Saver Bot </blockquote>
...
PRE_TEXT = """💰 **Premium Price**: Starting from $1 or 80 INR . **Study Ocean** (terms and conditions apply).
📥 **Download Limit**: Users can download up to 100 files in a single batch command.
🛑 **Batch**: You will get two modes /bulk and /batch. Users can Batch upto 10,000 foles at once.
...
async def plan_command(event):
    ...
    buttons = [
        [Button.url("Buy Premium", url="https://t.me/study_ocean_bot")]
    ]
...
async def term_command(event):
    # Creating inline keyboard with buttons
    buttons = [
        [Button.url("Query?", url="https://t.me/study_ocean_bot"),
         Button.url("Channel", url="https://t.me/+bxa7pKmXMCFjMjJl")]
    ]
...
async def youtube_dl_command(_, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            caption = f"{video_info['title']}\n\n__**Powered by [Sabka Content Saver Bot](https://t.me/Sabka_saver_bot)**__"  # Set caption to the title of the video
